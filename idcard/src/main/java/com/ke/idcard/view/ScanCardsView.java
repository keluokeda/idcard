...
import com.ke.idcard.OcrRecogTask;
import com.ke.idcard.R;
import com.ke.idcard.ThreadManager;
import com.ke.idcard.utils.CardScreenUtil;
import com.ke.idcard.utils.ManageIDCardRecogResult;
import kernal.idcard.android.Frame;
import kernal.idcard.android.RecogService;
import kernal.idcard.android.ResultMessage;
import kernal.idcard.camera.*;
...
public class ScanCardsView extends RelativeLayout implements Camera.PreviewCallback, ScanICamera {
    ...
    private Handler recogErrorHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    iScanReturnMessage.scanOCRIdCardError(errorMessage);
                    break;
                case 1:
                    ViewfinderView.FRAMECOLOR = Color.rgb(238, 65, 86);
                    text_tips.setText(getContext().getString(getResources().getIdentifier("please_place",
    ...
    int[] getNV21Point() {
        ...
        if (CardScreenUtil.ORIENTATION_PORTRAIT == CardScreenUtil.getScreenOrientation(getContext()) && !ViewfinderView.isSameScreen) {
            NV21Point[0] = (int) (viewfinderView.getFrame().top * mPreview.getScale());
            NV21Point[1] = (int) (viewfinderView.getFrame().left * mPreview.getScale());
            NV21Point[2] = (int) (viewfinderView.getFrame().bottom * mPreview.getScale());
            NV21Point[3] = (int) (viewfinderView.getFrame().right * mPreview.getScale());
        } else {
            NV21Point[0] = (int) (viewfinderView.getFrame().left * mPreview.getScale());
            NV21Point[1] = (int) (viewfinderView.getFrame().top * mPreview.getScale());
            NV21Point[2] = (int) (viewfinderView.getFrame().right * mPreview.getScale());
            NV21Point[3] = (int) (viewfinderView.getFrame().bottom * mPreview.getScale());
        }
        return NV21Point;
    }
     ...
     */
    @Override
    public void recogErrorUpdateUi(ResultMessage resultMessage) {
