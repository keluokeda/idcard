...
import com.ke.idcard.view.ScanCardsView;
import com.ke.idcard.view.ViewfinderView;
...
//        intent.putExtra("resultbundle", bundle);
        ...
        if (mainId == 2) {
            ...
            intent.putExtra("result", IDCardResult.Companion.fromResultMessage(resultMessage));

        } else if (mainId == 6) {
            //行驶证
            intent.putExtra("result", VehicleLicenseResult.Companion.fromResultMessage(resultMessage));
        }

        logMessage("main id = " + mainId);
     ...
     * 扫描识别失败，界面的跳转(回调接口)
     *
     * @param error -10601 开发码错误,把该文件中的devcode替换
    ...
    public void scanOCRIdCardError(String error) {
        Intent intent = new Intent();
        RecognitionError recognitionError = new RecognitionError(error);
        intent.putExtra("result", recognitionError);
        this.setResult(Activity.RESULT_OK, intent);
        this.finish();
    }
...
//            }
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        Log.i("string", "屏幕的宽度为:" + width + "---------" + "高度为:" + height);
